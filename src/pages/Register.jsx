...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      login({ _id: Date.now().toString(), name, email, role: 'user', favorites: [] }, 'mock-token-' + Date.now())
      navigate('/')
    } catch {
      ...
      <motion.div initial="hidden" animate="visible" variants={scaleIn} className="w-full max-w-md relative z-10">
        <div className="bg-white dark:bg-surface-800/80 dark:backdrop-blur-xl rounded-3xl p-8 shadow-premium border border-surface-100 dark:border-surface-700/60">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-surface-900 dark:text-white font-heading">{t('register.title', 'Create an account')}</h1>
            <p className="text-surface-500 dark:text-slate-400 text-sm mt-1">{t('register.subtitle', 'Start your journey with TravelScape')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <FiUser className="absolute ltr:left-4 rtl:right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400 dark:text-slate-400" />
                <input
            ...
            <div>
              <label className="block text-sm font-semibold text-surface-700 dark:text-slate-300 mb-1.5">{t('register.confirmPassword', 'Confirm Password')}</label>
              <div className="relative">
                <FiLock className="absolute ltr:left-4 rtl:right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400 dark:text-slate-400" />
                <input
