...
import { FiHeart, FiMapPin, FiStar, FiWifi, FiCheck } from 'react-icons/fi'
  ...
  return (
    ...
    >
      ...
      <div className="relative h-52 overflow-hidden">
        ...
        {/* Rating badge */}
        <div className="absolute top-4 left-4 flex items-center gap-1.5 px-3 py-1.5 bg-white/95 dark:bg-surface-900/90 backdrop-blur-md rounded-xl border border-white/20 dark:border-white/10 shadow-sm">
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {hotel.amenities.slice(0, 4).map((amenity) => {
            ...
            return (
              <span
                key={amenity}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-surface-50 dark:bg-surface-800/60 border border-surface-100 dark:border-surface-700/50 rounded-lg text-xs font-medium text-surface-600 dark:text-slate-300 transition-colors"
              >
          ...
          {hotel.amenities.length > 4 && (
            <span className="px-2 py-1 text-xs font-semibold text-primary-500 dark:text-primary-400">
        ...
        {/* Price & CTA */}
        <div className="flex items-center justify-between pt-3 border-t border-surface-100 dark:border-surface-700/80">
          <div>
            <span className="text-xl font-bold text-surface-900 dark:text-white">${hotel.pricePerNight}</span>
            <span className="text-sm text-surface-500 dark:text-slate-400"> / night</span>
          </div>
          <Link
            to={`/hotels/${hotel._id}`}
            className="px-4 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-primary-500 to-accent-500 hover:from-primary-600 hover:to-accent-600 rounded-xl shadow-md shadow-primary-500/10 hover:shadow-primary-500/25 transition-all duration-200 active:scale-95"
          >
            View Details
          </Link>
        </div>
